...
- **Send/ack timing.** `transact()` writes with `IOHIDDeviceSetReport`, sleeps ~31ms, then reads the reply; status `0x01` means "busy" and is retried once. A missing/unreadable GetReport response is treated as success (some dongle firmware doesn't reply), so `send()` — the write-only wrapper around it — reports no error. Reads must therefore check `answers(_:)` on the reply rather than trusting that one arrived. Don't tighten these sleeps without hardware testing.
- **Battery polling.** A `DispatchSourceTimer` on the same `queue` reads charge level (class `0x07`/`0x80`) once a minute, plus on connect and after each successful apply. Charging is *not* read from the device — the `0x07`/`0x84` status command answers unreliably on this model, so `updateActiveDevice()` derives it from the wired interface (`0x00AA`) being present in `controls`, which only happens while the cable is attached. Sharing the serial queue with `drain()` is what keeps polls from overlapping a write. A poll that gets no answer is normal (the mouse sleeps): it keeps the last reading and must never raise a user-facing error, which is why battery updates go through `publish()` instead of `pushState(error:)`.
